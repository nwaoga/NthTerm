...
- ADO remains the story ledger for #175-#180, while GitHub tags/releases remain the artifact delivery path.
...
- RC1 used version `0.1.0-rc.1` and remained intentionally unsigned. The release bar is a repeatable local verification command plus tag-triggered CI artifacts; Authenticode is a separate capability that requires a real certificate and stays out of the default build path.
- `0.1.0-rc.2` is the stacked-layout / glass / macOS packaging candidate. Until signing lands, upgrades are manual install-over (NSIS/dmg) with AppData / Application Support preserved; `electron-updater` is deferred to a post-signing story.
...
- #139 macOS smoke is automated on GitHub `macos-latest` via `scripts/smoke-macos.sh` instead of requiring a human Mac session for the packaged launch path. Quarantine xattr clear is the CI stand-in for Gatekeeper right-click Open. Focus/Overview interaction remains covered by #140 unit/component specs rather than brittle UI automation in the smoke script.
