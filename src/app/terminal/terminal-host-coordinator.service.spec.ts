...
describe('TerminalHostCoordinatorService', () => {
    ...
    let terminal: Pick<TerminalSessionService, 'setTerminalHosts' | 'restorePaneSessions'>;
    let restoreCalls = 0;
    ...
    beforeEach(() => {
        ...
        terminal = {
            setTerminalHosts: vi.fn().mockName('TerminalSessionService.setTerminalHosts'),
            restorePaneSessions: vi.fn().mockName('TerminalSessionService.restorePaneSessions'),
        };
        (terminal.restorePaneSessions as ReturnType<typeof vi.fn>).mockImplementation(async () => {
            restoreCalls += 1;
            inFlightRestores += 1;
