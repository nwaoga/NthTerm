import { DRACULA_ANSI, isDarkBackground, isTerminalAnsiPaletteId, resolveTerminalAnsiPalette, VS_CODE_DARK_ANSI, VS_CODE_LIGHT_ANSI, } from './terminal-ansi-palettes';
...
describe('terminal-ansi-palettes', () => {
    it('selects vscode palettes automatically from the terminal background', () => {
        ...
        expect(isDarkBackground('#0d1320')).toBe(true);
    ...
    it('validates palette ids', () => {
        expect(isTerminalAnsiPaletteId('nord')).toBe(true);
        expect(isTerminalAnsiPaletteId('invalid')).toBe(false);
    });
});
