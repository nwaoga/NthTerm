...
const LEGACY_WINDOW_TRANSPARENCY_PREFERENCE_KEY = 'nthterm.preferences.windowTransparency';
...
export class AppPreferencesService {
  readBottomPanelVisible(): boolean {
    try {
      return localStorage.getItem(BOTTOM_PANEL_PREFERENCE_KEY) === 'true';
    } catch {
      return false;
    }
  }
  ...
  readInspectorPanelVisible(): boolean {
    try {
      return localStorage.getItem(INSPECTOR_PANEL_PREFERENCE_KEY) === 'true';
    } catch {
      return false;
    }
  }
  ...
  readLeftRailVisible(): boolean {
    try {
      return localStorage.getItem(LEFT_RAIL_PREFERENCE_KEY) === 'true';
    } catch {
      return false;
    }
  }
  ...
  /** Drop the removed window-transparency preference so old installs stay opaque. */
  clearLegacyWindowTransparency(): void {
    try {
      localStorage.removeItem(LEGACY_WINDOW_TRANSPARENCY_PREFERENCE_KEY);
    } catch {
      // Preference persistence is best-effort only.
    }
  }
