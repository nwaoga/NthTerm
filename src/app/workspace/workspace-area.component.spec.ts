...
describe('WorkspaceAreaComponent', () => {
    ...
    const workspaceService = {
        ...
        createTerminalDraft: vi.fn().mockName('createTerminalDraft').mockReturnValue(null),
        addTerminal: () => undefined,
        persistWorkspaceState: async () => undefined,
        duplicateTerminal: vi.fn().mockName('duplicateTerminal').mockResolvedValue(null),
        hasRunningTerminals: () => true,
        removeTerminal: vi.fn().mockName('removeTerminal').mockResolvedValue(runtimeTerminal),
        focusTerminal: vi.fn().mockName('focusTerminal').mockResolvedValue('unchanged'),
        ...
        isTerminalRunning: vi.fn().mockName('isTerminalRunning').mockReturnValue(true),
        ...
        updateFocusedTerminalShell: vi.fn().mockName('updateFocusedTerminalShell'),
        updateFocusedTerminalName: vi.fn().mockName('updateFocusedTerminalName'),
        updateTerminalName: vi.fn().mockName('updateTerminalName'),
        updateTerminalThemeColors: vi.fn().mockName('updateTerminalThemeColors'),
        resetTerminalTheme: vi.fn().mockName('resetTerminalTheme'),
        updateFocusedTerminalThemeColors: vi.fn().mockName('updateFocusedTerminalThemeColors'),
        resetFocusedTerminalTheme: vi.fn().mockName('resetFocusedTerminalTheme'),
        resolveNewTerminalShell: vi.fn().mockName('resolveNewTerminalShell').mockImplementation((explicitShell, appDefaultShell) => explicitShell ?? appDefaultShell),
        updateWorkspaceShellProfile: vi.fn().mockName('updateWorkspaceShellProfile').mockResolvedValue(undefined),
        getShellOptions: () => [
    ...
    const terminalService = {
        ...
        relaunchTerminal: vi.fn().mockName('relaunchTerminal'),
        interruptTerminal: vi.fn().mockName('interruptTerminal'),
        killTerminal: vi.fn().mockName('killTerminal'),
        reattachTerminalSession: vi.fn().mockName('reattachTerminalSession'),
        focusTerminal: vi.fn().mockName('focusTerminal'),
        applyTerminalTheme: vi.fn().mockName('applyTerminalTheme'),
        syncTerminalSize: () => undefined,
        setInteractiveTerminalId: vi.fn().mockName('setInteractiveTerminalId'),
    ...
    const inspectorPresenter = {
        ...
        getInspectorItems: () => inspectorPresenter.activeTab === 'terminal'
            ? [
    ...
    const hostCoordinator = {
        ...
        syncAndRestore: vi.fn().mockName('syncAndRestore').mockResolvedValue(undefined),
    };

    beforeEach(async () => {
        inspectorPresenter.activeTab = 'workspace';
        workspaceService.isTerminalRunning.mockClear();
        workspaceService.isTerminalRunning.mockReturnValue(true);
        workspaceService.updateFocusedTerminalName.mockClear();
        workspaceService.updateTerminalName.mockClear();
        workspaceService.removeTerminal.mockClear();
        ...
        workspaceService.focusTerminal.mockClear();
        workspaceService.focusTerminal.mockResolvedValue('unchanged');
    ...
    it('shows an empty-state prompt when the workspace has no terminals', () => {
        ...
        vi.spyOn(component as any, 'getActiveTerminals').mockReturnValue([]);
        fixture.detectChanges();
    ...
    it('opens overview with preview cards and returns to focus when a card is selected', async () => {
        ...
        workspaceService.getActiveTabTerminals = () => terminals;
        workspaceService.terminals = terminals;
        workspaceService.focusTerminal.mockResolvedValue(terminals[2]);
        const fixture = TestBed.createComponent(WorkspaceAreaComponent);
        fixture.detectChanges();
    ...
    it('toggles overview with Ctrl+\\\\ and jumps terminals with Ctrl+number', () => {
        ...
        workspaceService.getActiveTabTerminals = () => terminals;
        workspaceService.terminals = terminals;
        workspaceService.focusTerminal.mockImplementation(async (id: string) => {
    ...
    it('renames the focused terminal from terminal settings', () => {
        ...
        expect(workspaceService.updateFocusedTerminalName).toHaveBeenCalledTimes(1);

        expect(workspaceService.updateFocusedTerminalName).toHaveBeenCalledWith('API Server');
    ...
    it('renames a terminal from the focus header by double-click', async () => {
        ...
        fixture.debugElement.query(By.css('.terminal-title-button')).nativeElement.dispatchEvent(new MouseEvent('dblclick', { bubbles: true }));
        fixture.detectChanges();
    ...
    it('uses the workspace shell resolver when adding a terminal without an explicit shell', async () => {
        ...
        (workspaceService.createTerminalDraft as any) = vi.fn().mockName('createTerminalDraft').mockReturnValue(draft);
        const fixture = TestBed.createComponent(WorkspaceAreaComponent);
        fixture.detectChanges();
    ...
    it('hides and restores the inspector rail through panel visibility events', () => {
        ...
        fixture.componentInstance.inspectorPanelVisibleChange.subscribe((visible) => visibilityChanges.push(visible));
        fixture.detectChanges();
