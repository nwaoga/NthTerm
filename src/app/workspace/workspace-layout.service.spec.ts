...
describe('WorkspaceLayoutService', () => {
    ...
    let workspace: {
        ...
        getActiveTabTerminals: Mock;
        getTerminalDisplayTitle: Mock;
    };
    ...
    beforeEach(() => {
        workspace = {
            ...
            getActiveTabTerminals: vi.fn().mockName('getActiveTabTerminals').mockReturnValue([
            ...
            getTerminalDisplayTitle: vi.fn().mockName('getTerminalDisplayTitle').mockImplementation((terminal: {
                name?: string;
            }, index: number) => terminal.name || `Terminal ${index + 1}`),
        };
    ...
    it('defaults to focus mode with zoomLevel 0', () => {
        ...
        expect(service.isFocusMode()).toBe(true);
    });
    ...
    it('toggles overview and rebinds workspace id on switch', () => {
        ...
        expect(service.isOverviewMode()).toBe(true);
    ...
    it('steps across the overview grid with wrapping', () => {
        workspace.getActiveTabTerminals.mockReturnValue([
            { id: 'terminal-1', name: 'One', cwd: 'C:\\a', shell: '', startupCommand: '', status: 'idle' },
            { id: 'terminal-2', name: 'Two', cwd: 'C:\\b', shell: '', startupCommand: '', status: 'idle' },
            { id: 'terminal-3', name: 'Three', cwd: 'C:\\c', shell: '', startupCommand: '', status: 'idle' },
