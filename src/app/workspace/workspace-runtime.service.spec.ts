...
function makeTerminal(overrides: Partial<RuntimeTerminal> & {
    id: string;
}): RuntimeTerminal {
    return {
        name: '',
...
describe('WorkspaceRuntimeService', () => {
    ...
    beforeEach(() => {
        workspaceBridge = {
            ...
            saveWorkspace: vi.fn().mockName('saveWorkspace').mockImplementation(async (draft: any) => makeWorkspace({
                ...draft,
                id: draft.id || 'ws-1',
                updatedAt: '2026-07-02T00:00:00.000Z',
                sessionSnapshot: draft.sessionSnapshot,
            })),
            createWorkspace: vi.fn().mockName('createWorkspace').mockImplementation(async (draft: any) => makeWorkspace({
                id: 'ws-created',
                shell: '',
            ...
            setActiveWorkspace: vi.fn().mockName('setActiveWorkspace').mockResolvedValue(makeWorkspace({
                id: 'ws-2',
                name: 'Workspace 2',
                cwd: 'C:\\Projects\\Two',
                accent: 'violet',
    ...
    it('blocks createTerminalDraft and duplicateTerminal at the workspace limit', async () => {
        service.activeWorkspace = makeWorkspace();
        service.selectedWorkspaceId = 'ws-1';
        service.terminals = Array.from({ length: MAX_TERMINALS_PER_WORKSPACE }, (_, index) => makeTerminal({
            id: `terminal-${index}`,
    ...
    it('removes, focuses, and cycles terminals', async () => {
        ...
        expect(await service.focusTerminal('terminal-2')).toEqual(expect.objectContaining({ id: 'terminal-2' }));
        ...
        expect((await service.cycleTerminal(1)) as RuntimeTerminal).toEqual(expect.objectContaining({ id: 'terminal-3' }));
        expect((await service.cycleTerminal(-1)) as RuntimeTerminal).toEqual(expect.objectContaining({ id: 'terminal-2' }));

        const removed = await service.removeTerminal('terminal-2');
        expect(removed).toEqual(expect.objectContaining({ id: 'terminal-1' }));
    ...
    it('applyWorkspace loads flat terminals and migrates multi-tab snapshots to active-tab terminals only', () => {
        service.applyWorkspace(makeWorkspace({
            shell: 'cmd',
            sessionSnapshot: {
                layout: {
                    mode: 'grid-2',
        ...
        expect(service.workspaceSummary.paneCount).toBe(2);
        expect(service.workspaceSummary.layoutMode).toBe('grid-2');
        expect((service.workspaceSummary as {
            tabCount?: number;
        }).tabCount).toBeUndefined();
    ...
    it('adds WSL distros to shell options and resolves WSL profiles', () => {
        ...
        try {
            ...
            expect(service.getWorkspaceShellProfileOptions().map((option) => option.label)).toContain('WSL: Debian');
            expect(service.getWorkspaceShellProfileOptions().map((option) => option.label)).toContain('System Default (PowerShell)');
    ...
    it('createWorkspace writes a flat empty terminals snapshot', async () => {
        ...
        const payload = vi.mocked(workspaceBridge.createWorkspace).mock.lastCall![0];
    ...
    it('persists workspace shell profile updates', async () => {
        ...
        expect(workspaceBridge.saveWorkspace).toHaveBeenCalledWith(expect.objectContaining({ shell: 'powershell' }));
    });

    it('loadReferencePreviewState has four terminals', () => {
        const workspace = service.loadReferencePreviewState();

        expect(service.previewMode).toBe(true);
        ...
        expect(service.isTerminalRunning(terminal)).toBe(true);
    ...
    it('keeps real workspace selection out of preview mode', async () => {
        ...
        expect(service.previewMode).toBe(false);
